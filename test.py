...
ADMIN_CODE = "02110240"
PREMIUM_ADMIN_CODE = "1309"  # ← MOT DE PASSE SUPPLÉMENTAIRE GESTION PREMIUM
...
def get_modeles_disponibles(api_key):
    ...
    try:
        ...
        def priorite(nom):
            ...
            return 3
        return sorted(modeles, key=priorite)
    except Exception as e:
...
Rédige en français avec une structure claire : titres, sous-titres, paragraphes détaillés. Sois exhaustif et professionnel."""
        ...
        for modele in modeles:
            try:
                ...
                erreurs.append(f"{modele} → {type(e).__name__}: {e}"); continue

        return f"❌ Gemini indisponible. Détails : {' | '.join(erreurs)}"
    except Exception as e:
...
def creer_docx(contenu, service, client_nom):
    ...
    run_t.font.color.rgb = RC(0x1F, 0x4E, 0x79); run_t.font.name = "Arial"

    p_info = doc.add_paragraph()
    p_info.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run_i = p_info.add_run(f"Client : {client_nom}     |     Généré le : {datetime.now().strftime('%d/%m/%Y à %H:%M')}")
    run_i.font.size = Pt(10); run_i.font.color.rgb = RC(0x7F, 0x7F, 0x7F)
    run_i.font.name = "Arial"; run_i.italic = True
    ...
    def add_formatted_para(doc, text, style_name="Normal", bold=False, size=11, color=None, align=None):
        ...
        for part in parts:
            if part.startswith("**") and part.endswith("**"):
                ...
                run = p.add_run(clean); run.bold = bold
            run.font.name = "Arial"; run.font.size = Pt(size)
            if color: run.font.color.rgb = RC(*color)
        return p
    ...
    def nettoyer_formule(f):
        f = f.replace(r"\,", " ").replace(r"\text{", "").replace("}", "")
        f = f.replace(r"\frac{", "(").replace("}{", ")/(").replace(r"\sqrt{", "sqrt(")
        f = f.replace(r"\omega", "omega").replace(r"\varphi", "phi").replace(r"\pi", "pi")
        f = f.replace(r"\cdot", "x").replace(r"\times", "x").replace(r"\approx", "≈")
        f = f.replace(r"\left(", "(").replace(r"\right)", ")").replace("\\\\", "")
    ...
    while i < len(lignes):
        ...
        if l.startswith("#### "): doc.add_heading(l[5:].strip(), level=4); i += 1; continue
        if l.startswith("### "):  doc.add_heading(l[4:].strip(), level=3); i += 1; continue
        if l.startswith("## "):   doc.add_heading(l[3:].strip(), level=2); i += 1; continue
        if l.startswith("# "):    doc.add_heading(l[2:].strip(), level=1); i += 1; continue
        ...
        if l.startswith("|") and l.endswith("|"):
            ...
            if table_lines:
                ...
                col_widths_map = {2: [3.0, 6.0], 3: [1.0, 7.5, 2.5], 4: [1.0, 5.0, 2.5, 2.5]}
                ...
                for r_idx, row_data in enumerate(table_lines):
                    ...
                    for c_idx, cell_text in enumerate(row_data):
                        ...
                        if c_idx < len(col_widths): cell.width = DocxCm(col_widths[c_idx])
                        tc = cell._tc; tcPr = tc.get_or_add_tcPr()
                        tcMar = _OE("w:tcMar")
                        for side in ["top","bottom","left","right"]:
                            m = _OE(f"w:{side}"); m.set(_qn("w:w"), "120"); m.set(_qn("w:type"), "dxa"); tcMar.append(m)
                        tcPr.append(tcMar)
                        if is_header:
                            shd = _OE("w:shd"); shd.set(_qn("w:val"), "clear"); shd.set(_qn("w:color"), "auto"); shd.set(_qn("w:fill"), "1F4E79"); tcPr.append(shd)
                        elif r_idx % 2 == 0:
                            shd = _OE("w:shd"); shd.set(_qn("w:val"), "clear"); shd.set(_qn("w:color"), "auto"); shd.set(_qn("w:fill"), "EEF3FA"); tcPr.append(shd)
                        ...
                        run.font.name = "Arial"; run.font.size = Pt(10); run.bold = is_header
                        if is_header: run.font.color.rgb = RC(0xFF, 0xFF, 0xFF)
                doc.add_paragraph("")
            continue
        ...
        if not l.strip(): doc.add_paragraph(""); i += 1; continue

        if l.strip().startswith("**") and l.strip().endswith("**") and l.strip().count("**") == 2:
            texte = l.strip()[2:-2]
            p = doc.add_paragraph()
            run = p.add_run(texte); run.bold = True; run.font.name = "Arial"
            run.font.size = Pt(12); run.font.color.rgb = RC(0x1F, 0x4E, 0x79)
            i += 1; continue
...
def creer_xlsx(description, client_nom):
    ...
    wb = Workbook()
    BLEU_FONCE = "1F4E79"; BLEU_CLAIR = "BDD7EE"; BLANC = "FFFFFF"; GRIS = "F2F2F2"
    ...
    ws1 = wb.active; ws1.title = "Saisie Dépenses"; ws1.sheet_view.showGridLines = False
    ws1.merge_cells("A1:H1"); t = ws1["A1"]
    t.value = f"SUIVI DES DÉPENSES — JANVIER 2026  |  {client_nom}"
    hdr(t, size=13); ws1.row_dimensions[1].height = 34
    ws1.merge_cells("A2:H2"); ws1["A2"].value = f"Généré le {datetime.now().strftime('%d/%m/%Y')} — Nova AI"
    ...
    for r,row in enumerate(rows,4):
        ...
        for c,val in enumerate(row,1):
            ...
            cell.font = Font(name="Arial",size=10,bold=(c==6),color=("1F4E79" if c==6 else "000000"))
            cell.fill = PatternFill("solid",start_color=bg)
            cell.alignment = Alignment(vertical="center",horizontal="center" if c in [1,2,6,7] else "left")
            brd(cell)
            if c==6: cell.number_format = '#,##0 "FCFA"'

    tr = len(rows)+4
    ws1.merge_cells(f"A{tr}:E{tr}"); hdr(ws1[f"A{tr}"],size=11); ws1[f"A{tr}"].value="TOTAL JANVIER"; brd(ws1[f"A{tr}"])
    tot = ws1[f"F{tr}"]; tot.value=f"=SUM(F4:F{tr-1})"; tot.number_format='#,##0 "FCFA"'; hdr(tot); brd(tot)
    ws1.row_dimensions[tr].height=28

    ws2 = wb.create_sheet("Catégories"); ws2.sheet_view.showGridLines = False
    ...
    cats=[("Alimentation",50000),("Transport",20000),("Factures",30000),("Restauration",15000),
          ("Communication",15000),("Santé",20000),("Habillement",25000),("Logement",150000),
          ("Loisirs",10000),("Autres",10000)]
    for r,(cat,budget) in enumerate(cats,3):
        ...
        c2.font=Font(name="Arial",size=10,color="0000FF"); c2.fill=PatternFill("solid",start_color=bg)
        c2.alignment=Alignment(horizontal="center"); brd(c2)
        ...
        c4.fill=PatternFill("solid",start_color=bg); c4.alignment=Alignment(horizontal="center"); brd(c4); c4.font=Font(name="Arial",size=10)
    tr2=len(cats)+3; ws2.cell(row=tr2,column=1,value="TOTAL"); hdr(ws2.cell(row=tr2,column=1)); brd(ws2.cell(row=tr2,column=1))
    for col in [2,3,4]:
    ...
    ws3 = wb.create_sheet("Tableau de Bord"); ws3.sheet_view.showGridLines = False
    ws3.merge_cells("A1:E1"); t3=ws3["A1"]; t3.value="TABLEAU DE BORD — JANVIER 2026"; hdr(t3,size=13); ws3.row_dimensions[1].height=34
    ws3.column_dimensions["A"].width=26; ws3.column_dimensions["B"].width=22
    ws3.column_dimensions["C"].width=4; ws3.column_dimensions["D"].width=26; ws3.column_dimensions["E"].width=22
    kpis=[("Total Dépenses Janvier","='Saisie Dépenses'!F14","1F4E79"),("Budget Total Prévu","='Catégories'!B13","2E75B6"),
          ("Écart Budget/Réel","='Catégories'!D13","375623"),("Nb de Transactions","=COUNTA('Saisie Dépenses'!A4:A1000)-1","7F7F7F")]
    positions=[(3,1,2),(3,4,5),(6,1,2),(6,4,5)]
    for (row,cl,cv),(label,formula,bg) in zip(positions,kpis):
        lc=ws3.cell(row=row,column=cl,value=label); lc.font=Font(bold=True,name="Arial",size=11,color=BLANC)
        lc.fill=PatternFill("solid",start_color=bg); lc.alignment=Alignment(horizontal="center",vertical="center"); ws3.row_dimensions[row].height=32
        vc=ws3.cell(row=row,column=cv,value=formula); vc.number_format='#,##0 "FCFA"' if "Nb" not in label else "0"
        vc.font=Font(bold=True,name="Arial",size=13,color=BLANC); vc.fill=PatternFill("solid",start_color=bg)
        vc.alignment=Alignment(horizontal="center",vertical="center")
    ws3.merge_cells("A9:E9"); rh=ws3["A9"]; rh.value="RÉCAPITULATIF PAR CATÉGORIE"; hdr(rh,bg="2E75B6",size=12); ws3.row_dimensions[9].height=28
    ...
    for r,cat in enumerate([c for c,_ in cats],11):
        cat_row=r-8; bg=BLEU_CLAIR if r%2==0 else BLANC
        c1=ws3.cell(row=r,column=1,value=cat); c1.font=Font(name="Arial",size=10,bold=True)
        c1.fill=PatternFill("solid",start_color=bg); c1.alignment=Alignment(vertical="center"); brd(c1)
        for col,formula in enumerate([f"='Catégories'!B{cat_row+2}",f"='Catégories'!C{cat_row+2}",f"='Catégories'!D{cat_row+2}",f"=IF(B{r}=0,0,C{r}/B{r})"],2):
            cell=ws3.cell(row=r,column=col,value=formula); cell.font=Font(name="Arial",size=10)
            cell.fill=PatternFill("solid",start_color=bg); cell.alignment=Alignment(horizontal="center"); brd(cell)
...
# ← NOUVEAU : état du verrou Premium admin
if "premium_admin_unlocked" not in st.session_state:
    st.session_state["premium_admin_unlocked"] = False

# Reconnaissance automatique via cookie navigateur
...
if st.session_state["current_user"]:
    ...
    components.html(f"""
        <script>localStorage.setItem('nova_user_id', '{uid_connecte}');</script>
    """, height=0)


# ==========================================
# CSS — THÈME BLEU (STANDARD) + THÈME OR (PREMIUM)
# ==========================================

def inject_custom_css(is_premium=False):
    # ── CSS COMMUN AUX DEUX THÈMES ──────────────────────────────────
    commun = """
        ...
        * { font-family: 'Poppins', sans-serif; }
        ...
        .logo-item {
            ...
            filter: grayscale(0.5) opacity(0.7); transition: all 0.3s ease;
        }
        .logo-item:hover {
        ...
        .stTabs [data-baseweb="tab"] {
            height: 60px; white-space: pre-wrap;
            border-radius: 10px; font-weight: 700 !important;
            font-size: 1.2rem !important; transition: all 0.3s ease; padding: 0 25px;
        }
        .stTabs [data-baseweb="tab"]:hover { transform: translateY(-2px); }

        .stTextInput label, .stSelectbox label, .stTextArea label {
            font-weight: 600 !important; font-size: 1.1rem !important; margin-bottom: 5px;
        }
        div[data-baseweb="input"], div[data-baseweb="select"] > div {
            color: white !important; border-radius: 10px !important;
        }
        .stTextArea textarea {
            color: white !important; border-radius: 10px !important; transition: transform 0.3s;
        }
        .stTextArea textarea:focus { transform: scale(1.01); }

        .stButton > button {
            border-radius: 12px; padding: 0.8rem 2rem; width: 100%;
            margin-top: 10px; font-weight: 700; font-size: 1.1rem !important;
        ...
        .support-btn {
            ...
            border: 2px solid #25D366; color: #25D366 !important; padding: 10px;
        ...
        .gemini-card {
            ...
            border: 2px solid rgba(0,210,255,0.4); border-radius: 14px; padding: 16px 20px; margin: 12px 0;
        }
        .gemini-title { color: #00d2ff; font-weight: 800; font-size: 0.95rem; letter-spacing: 1px; }
        .gemini-sub   { color: rgba(255,255,255,0.5); font-size: 0.75rem; }
    """

    # ── THÈME BLEU — CLIENTS STANDARD ───────────────────────────────
    theme_bleu = """
        .stApp {
            background: #0f0c29;
        ...
        .main-title {
            ...
            font-weight: 800; font-size: 3.5rem !important; text-align: center;
            margin-bottom: 20px; text-shadow: 0 0 20px rgba(0,210,255,0.3);
        }

        .stTabs [data-baseweb="tab-list"] {
            gap: 20px; background-color: rgba(255,255,255,0.05);
            padding: 10px; border-radius: 15px; border: 1px solid rgba(255,255,255,0.1);
        }
        .stTabs [data-baseweb="tab"] {
            background-color: rgba(0,210,255,0.1); color: white !important;
            border: 1px solid transparent;
        }
        .stTabs [data-baseweb="tab"]:nth-child(2) {
            border: 1px solid #2ecc71 !important; box-shadow: 0 0 15px rgba(46,204,113,0.2);
            background-color: rgba(46,204,113,0.1);
        }
        .stTabs [aria-selected="true"] {
            background-color: rgba(0,210,255,0.6) !important;
            border: 1px solid #00d2ff !important; box-shadow: 0 0 20px rgba(0,210,255,0.4);
        }
        ...
        .stTextInput label, .stSelectbox label, .stTextArea label { color: #00d2ff !important; }
        div[data-baseweb="input"], div[data-baseweb="select"] > div {
        ...
        .stButton > button {
            ...
            color: white !important; box-shadow: 0 4px 10px rgba(0,210,255,0.3);
        }
        .stButton > button:hover { box-shadow: 0 6px 20px rgba(0,210,255,0.5); }
        ...
        .info-card {
            background: rgba(0,0,0,0.4) !important; border-left: 4px solid #00d2ff;
        ...
        .file-card { background: rgba(255,255,255,0.08); border: 2px solid rgba(46,204,113,0.5); }

        .premium-card {
            background: rgba(20,20,30,0.8); border: 2px solid #FFD700; border-radius: 20px;
            padding: 25px; text-align: center; margin-bottom: 30px;
            box-shadow: 0 0 30px rgba(255,215,0,0.2); position: relative; overflow: hidden;
        }
        .premium-card::before {
            content: ""; position: absolute; top: 0; left: 0; width: 100%; height: 5px;
            background: linear-gradient(90deg, #FFD700, #FF8C00, #FFD700);
        }
        .premium-title { color: #FFD700 !important; font-size: 1.5rem; font-weight: 800; text-transform: uppercase; margin-bottom: 10px; }
        .premium-desc  { color: #ffffff !important; font-size: 1rem; margin-bottom: 20px; line-height: 1.5; }

        .btn-gold {
            background: linear-gradient(45deg, #FFD700, #FF8C00); color: #000 !important;
            padding: 12px 30px; border-radius: 50px; text-decoration: none; font-weight: 800;
            font-size: 1.1rem; display: inline-block; box-shadow: 0 5px 15px rgba(255,215,0,0.4);
            transition: transform 0.2s, box-shadow 0.2s; border: none; cursor: pointer;
        }
        .btn-gold:hover { transform: scale(1.05); box-shadow: 0 8px 25px rgba(255,215,0,0.6); }
    """

    # ── THÈME OR — MEMBRES PREMIUM ───────────────────────────────────
    theme_or = """
        /* ════════════════════════════════════════════════════════
           THÈME OR COMPLET — MEMBRES PREMIUM NOVA GOLD
        ════════════════════════════════════════════════════════ */
        ...
        @keyframes gold-glow-pulse {
            0%,100% { box-shadow: 0 0 30px rgba(255,215,0,0.2), inset 0 0 30px rgba(255,215,0,0.02); }
            50%      { box-shadow: 0 0 80px rgba(255,215,0,0.5), inset 0 0 50px rgba(255,215,0,0.05); }
        }
        @keyframes gold-border-flow {
            0%   { border-color: #b8860b; box-shadow: 0 0 8px rgba(184,134,11,0.4); }
            33%  { border-color: #FFD700; box-shadow: 0 0 20px rgba(255,215,0,0.7); }
            66%  { border-color: #FFF5C0; box-shadow: 0 0 15px rgba(255,245,192,0.5); }
            100% { border-color: #b8860b; box-shadow: 0 0 8px rgba(184,134,11,0.4); }
        }
        ...
        @keyframes gold-title-shine {
            ...
            50%      { filter: brightness(1.3) drop-shadow(0 0 25px rgba(255,215,0,0.8)); }
        }

        /* FOND GLOBAL DORÉ */
        .stApp {
            background: radial-gradient(ellipse at 0% 0%, #1a1200 0%, #0d0900 30%, #000000 100%),
                        linear-gradient(135deg, #1a1200 0%, #0a0700 50%, #050300 100%) !important;
        ...
        .stApp::before {
            ...
            background:
                radial-gradient(circle at 15% 25%, rgba(255,215,0,0.04) 0%, transparent 35%),
                radial-gradient(circle at 85% 75%, rgba(255,140,0,0.03) 0%, transparent 30%),
                radial-gradient(circle at 50% 50%, rgba(255,215,0,0.02) 0%, transparent 60%);
        }

        /* TITRE PRINCIPAL DORÉ */
        .main-title {
            background: linear-gradient(90deg, #7a5500, #b8860b, #FFD700, #FFF5C0, #FFD700, #b8860b, #7a5500) !important;
            background-size: 300% auto !important;
            -webkit-background-clip: text !important;
            -webkit-text-fill-color: transparent !important;
            font-weight: 800 !important; font-size: 3.5rem !important; text-align: center !important;
            margin-bottom: 20px !important;
            animation: gold-shimmer 4s linear infinite, gold-title-shine 3s ease-in-out infinite !important;
            letter-spacing: 3px !important;
        }

        /* SIDEBAR DORÉE */
        [data-testid="stSidebar"] {
            background: linear-gradient(180deg, #120e00 0%, #0d0a00 50%, #080600 100%) !important;
            border-right: 1px solid rgba(255,215,0,0.25) !important;
        }
        [data-testid="stSidebar"]::before {
            content: '';
            position: absolute; top: 0; left: 0; width: 100%; height: 3px;
            background: linear-gradient(90deg, transparent, #FFD700, #FFF5C0, #FFD700, transparent);
        }

        /* TABS DORÉS */
        .stTabs [data-baseweb="tab-list"] {
            gap: 20px; padding: 10px; border-radius: 15px;
        ...
        .stTabs [aria-selected="true"] {
            background: linear-gradient(90deg, rgba(184,134,11,0.6), rgba(255,215,0,0.7)) !important;
        ...
        div[data-baseweb="input"], div[data-baseweb="select"] > div {
            border: 1px solid rgba(255,215,0,0.35) !important;
            background: rgba(10,8,0,0.7) !important;
        }
        ...
        /* TEXTAREA DORÉE */
        .stTextArea textarea {
            background: rgba(10,8,0,0.8) !important;
            border: 2px solid #b8860b !important;
            color: #FFF8DC !important;
            animation: gold-border-flow 3s ease-in-out infinite !important;
        }
        .stTextArea textarea:focus {
            animation: gold-border-flow 1.5s ease-in-out infinite !important;
        }
        ...
        .stButton > button {
            background: linear-gradient(90deg, #7a5500, #b8860b, #FFD700, #FFF5C0, #FFD700, #b8860b, #7a5500) !important;
            background-size: 300% auto !important;
            color: #0a0700 !important; font-weight: 800 !important;
            box-shadow: 0 4px 20px rgba(255,215,0,0.35) !important;
            animation: gold-shimmer 3s linear infinite !important;
        }
        .stButton > button:hover {
            box-shadow: 0 8px 35px rgba(255,215,0,0.6) !important;
            transform: translateY(-3px) scale(1.01) !important;
        }

        /* BARRE DE PROGRESSION DORÉE */
        .stProgress > div > div > div > div {
            background: linear-gradient(to right, #b8860b, #FFD700, #FFF5C0, #FFD700) !important;
        ...
        /* INFO CARD DORÉE */
        .info-card {
            background: rgba(10,8,0,0.6) !important;
            border-left: 4px solid #FFD700 !important;
            padding: 15px; border-radius: 0 10px 10px 0; margin-bottom: 15px;
            box-shadow: -2px 0 15px rgba(255,215,0,0.1);
        }
        .info-title {
            color: #FFD700 !important; font-weight: bold; font-size: 1.1rem;
            display: block; margin-bottom: 8px; text-transform: uppercase;
        }

        /* FILE CARD DORÉE */
        .file-card {
            background: linear-gradient(135deg, rgba(20,15,0,0.8), rgba(10,8,0,0.9)) !important;
            border: 1px solid rgba(255,215,0,0.4) !important;
            box-shadow: 0 4px 20px rgba(255,215,0,0.08), 0 0 0 1px rgba(255,215,0,0.05) !important;
            position: relative; overflow: hidden;
        }
        .file-card::before {
            content: ''; position: absolute; top: 0; left: 0; width: 4px; height: 100%;
            background: linear-gradient(180deg, #FFD700, #FF8C00, #b8860b, #FFD700);
        }

        /* BANNIÈRE PREMIUM ACTIVE DORÉE */
        .premium-banner-gold {
            background: linear-gradient(135deg, rgba(255,215,0,0.1), rgba(184,134,11,0.07));
            border: 2px solid #FFD700; border-radius: 20px;
            padding: 22px 28px; text-align: center; margin-bottom: 25px;
            position: relative; overflow: hidden;
            animation: gold-glow-pulse 3s ease-in-out infinite;
        }
        .premium-banner-gold::before {
            content: ''; position: absolute; top: 0; left: 0; right: 0; height: 3px;
            background: linear-gradient(90deg, transparent, #b8860b, #FFD700, #FFF5C0, #FFD700, #b8860b, transparent);
            background-size: 200% auto; animation: gold-shimmer 2.5s linear infinite;
        }
        .premium-banner-gold::after {
            content: ''; position: absolute; bottom: 0; left: 0; right: 0; height: 1px;
            background: linear-gradient(90deg, transparent, rgba(255,215,0,0.4), transparent);
        }
        /* Scanline animée sur la bannière */
        .premium-banner-gold .scanline {
            position: absolute; left: 0; right: 0; height: 2px;
            background: linear-gradient(90deg, transparent, rgba(255,215,0,0.15), transparent);
            animation: gold-scanline 4s linear infinite; pointer-events: none;
        }

        /* CARD UPGRADE DORÉE */
        .premium-card {
            background: linear-gradient(145deg, rgba(20,15,0,0.95), rgba(30,22,0,0.9)) !important;
            border: 2px solid #FFD700 !important; border-radius: 20px !important;
            padding: 25px !important; text-align: center !important; margin-bottom: 30px !important;
            box-shadow: 0 0 50px rgba(255,215,0,0.15) !important;
            animation: gold-glow-pulse 3s ease-in-out infinite !important;
        }
        .premium-card::before {
            background: linear-gradient(90deg, transparent, #b8860b, #FFD700, #FFF5C0, #FFD700, #b8860b, transparent) !important;
            background-size: 200% auto !important; animation: gold-shimmer 2.5s linear infinite !important;
        }
        .premium-title { color: #FFD700 !important; text-shadow: 0 0 10px rgba(255,215,0,0.5) !important; }
        .premium-desc  { color: #FFF8DC !important; }
        .btn-gold { box-shadow: 0 5px 20px rgba(255,215,0,0.5) !important; }
        .btn-gold:hover { box-shadow: 0 8px 35px rgba(255,215,0,0.8) !important; }
        ...
        .support-btn:hover {
            ...
            color: #000 !important; border-color: #FFD700 !important;
        }

        /* TEXTES GÉNÉRAUX */
        .stApp p, .stApp span, .stApp div, .stApp li { color: #FFF8DC; }
        ...
        [data-testid="stExpander"] {
            border: 1px solid rgba(255,215,0,0.3) !important;
            background: rgba(10,8,0,0.5) !important;
        }
        ...
        [data-testid="stMetricLabel"] { color: rgba(255,215,0,0.7) !important; }
        ...
        /* SUCCESS / ERROR / INFO */
        [data-testid="stSuccess"] { background: rgba(255,215,0,0.08) !important; border-left: 4px solid #FFD700 !important; }
        [data-testid="stInfo"]    { background: rgba(255,215,0,0.06) !important; border-left: 4px solid #b8860b !important; }

        /* RADIO BUTTONS */
        [data-testid="stRadio"] label { color: #FFD700 !important; }

        /* TOAST */
        .stToast { background: rgba(20,15,0,0.95) !important; border: 1px solid #FFD700 !important; }
    """

    css_final = commun + (theme_or if is_premium else theme_bleu)
    st.markdown(f"<style>{css_final}</style>", unsafe_allow_html=True)

    if st.session_state.get("is_glowing"):
...
def show_auth_page():
    ...
    .auth-logo-ring {
        ...
        font-size: 2.6rem; animation: glow-border 3s ease-in-out infinite; position: relative;
    }
    ...
    .auth-subtitle { color: rgba(255,215,0,0.65); font-size: 0.95rem; letter-spacing: 4px; text-transform: uppercase; animation: float-up 1s ease 0.5s both; margin-bottom: 6px; }
    .auth-tagline  { color: rgba(255,255,255,0.4); font-size: 0.82rem; letter-spacing: 1.5px; animation: float-up 1s ease 0.8s both; }

    .auth-divider { display: flex; align-items: center; gap: 14px; margin: 28px auto 32px auto; max-width: 420px; animation: float-up 1s ease 1s both; }
    .auth-divider-line { flex: 1; height: 1px; background: linear-gradient(90deg, transparent, rgba(255,215,0,0.5), transparent); }
    .auth-divider-dot  { width: 6px; height: 6px; border-radius: 50%; background: #FFD700; animation: pulse-dot 1.8s ease-in-out infinite; }

    .auth-card {
        background: linear-gradient(145deg, rgba(20,15,5,0.95), rgba(35,25,5,0.9));
        border: 1px solid rgba(255,215,0,0.35); border-radius: 22px;
        padding: 32px 28px 28px 28px; position: relative; overflow: hidden;
        animation: float-up 0.9s ease both; animation-delay: var(--card-delay, 0s);
    }
    .auth-card::before {
        content: ''; position: absolute; top: 0; left: 0; right: 0; height: 3px;
        background: linear-gradient(90deg, #b8860b, #FFD700, #fff5c0, #FFD700, #b8860b);
    ...
    .auth-card-icon {
        ...
        border: 1px solid rgba(255,215,0,0.3); box-shadow: 0 0 12px rgba(255,215,0,0.15);
    }
    .auth-card-title { color: #FFD700 !important; font-size: 1.15rem; font-weight: 800; letter-spacing: 0.5px; margin: 0; text-transform: uppercase; }
    .auth-card-desc  { color: rgba(255,255,255,0.35); font-size: 0.75rem; margin-top: 2px; letter-spacing: 0.5px; }

    .auth-page .stTextInput label { color: rgba(255,215,0,0.8) !important; font-size: 0.82rem !important; font-weight: 600 !important; letter-spacing: 1px; text-transform: uppercase; }
    .auth-page div[data-baseweb="input"] { background: rgba(0,0,0,0.5) !important; border: 1px solid rgba(255,215,0,0.25) !important; border-radius: 12px !important; transition: border-color 0.3s, box-shadow 0.3s !important; }
    .auth-page div[data-baseweb="input"]:focus-within { border-color: rgba(255,215,0,0.7) !important; box-shadow: 0 0 0 3px rgba(255,215,0,0.12) !important; }

    @keyframes btn-shimmer { 0% { background-position: -300% center; } 100% { background-position: 300% center; } }
    @keyframes btn-float {
        0%, 100% { transform: translateY(0px); box-shadow: 0 6px 25px rgba(255,215,0,0.45); }
        50%       { transform: translateY(-4px); box-shadow: 0 14px 35px rgba(255,215,0,0.65); }
    }
    .auth-page .stButton > button {
        background: linear-gradient(90deg, #7a5500, #b8860b, #FFD700, #fff5c0, #FFD700, #b8860b, #7a5500) !important;
        background-size: 300% auto !important; color: #0a0800 !important;
        font-weight: 800 !important; font-size: 0.92rem !important; letter-spacing: 2.5px !important;
        text-transform: uppercase !important; border-radius: 50px !important; border: none !important;
        padding: 0.82rem 1.8rem !important; animation: btn-shimmer 3s linear infinite, btn-float 3.5s ease-in-out infinite !important;
    }

    .auth-secure-badge {
        display: flex; align-items: center; justify-content: center; gap: 8px; margin-top: 28px;
        color: rgba(255,215,0,0.35); font-size: 0.72rem; letter-spacing: 1.5px; text-transform: uppercase;
            ...
            if st.form_submit_button("💎 REJOINDRE NOVA AI"):
                if new_uid and new_wa:
                    ...
                    if new_uid not in db["users"]:
                        db["users"][new_uid] = {
                            "whatsapp": normalize_wa(new_wa), "email": "Non renseigné",
        ...
        components.html(f"""
            ...
            (function() {{
                setTimeout(function() {{
                    ...
                    audio.volume = 1; audio.play().catch(function(e) {{}});
                }}, 3000);
            }})();
            </script>
        """, height=0)
...
def main_dashboard():
    ...
    # ── INJECTION THÈME SELON STATUT ────────────────────────────────
    inject_custom_css(is_premium=premium_actif)
    ...
    # ── TITRE ───────────────────────────────────────────────────────
    if premium_actif:
        st.markdown("<h1 class='main-title'>✦ NOVA AI GOLD ✦</h1>", unsafe_allow_html=True)
    else:
    ...
    # ── BANNIÈRE PREMIUM ACTIF ou CARD UPGRADE ──────────────────────
    ...
    if premium_actif and premium_info:
        st.markdown(f"""
        <div class="premium-banner-gold">
            <div class="scanline"></div>
            <div style="font-size:1.5rem; font-weight:800; color:#FFD700; letter-spacing:2px; text-shadow:0 0 15px rgba(255,215,0,0.5);">
                ✦ MEMBRE NOVA GOLD — {premium_info['plan'].upper()} ✦
            </div>
            <div style="color:rgba(255,215,0,0.75); margin-top:8px; font-size:0.95rem;">
                ⚡ Génération IA instantanée activée &nbsp;·&nbsp;
                Expire le <b>{premium_info['expiry']}</b> ({premium_info['jours_restants']} jours restants)
            </div>
        </div>""", unsafe_allow_html=True)
    else:
        st.markdown("""
        ...
        with col_btn_center:
            if st.button("💎 ACTIVER NOVA PREMIUM", key="open_premium"):
                st.session_state["show_premium_modal"] = True; st.rerun()

    if st.session_state["show_premium_modal"]:
        st.markdown("""
            <div style="background:linear-gradient(135deg,#1a1a2e,#16213e,#0f3460);border:2px solid #FFD700;
                        border-radius:24px;padding:35px 25px 30px 25px;margin:10px 0 30px 0;
        ...
        with col1_p:
            st.markdown("""<div style="background:rgba(255,255,255,0.05);border:1px solid rgba(255,215,0,0.4);border-radius:18px;padding:28px 16px;text-align:center;min-height:300px;">
        ...
        with col2_p:
            st.markdown("""<div style="background:linear-gradient(135deg,rgba(0,210,255,0.15),rgba(58,123,213,0.15));border:2px solid #00d2ff;border-radius:18px;padding:28px 16px;text-align:center;min-height:300px;position:relative;">
                ...
                <div style="background:rgba(0,210,255,0.1);border-radius:10px;padding:10px;">
        ...
        with col3_p:
            st.markdown("""<div style="background:rgba(255,255,255,0.05);border:1px solid rgba(46,204,113,0.4);border-radius:18px;padding:28px 16px;text-align:center;min-height:300px;">
                <div style="font-size:2.5rem;margin-bottom:10px;">👑</div>
        ...
        with col_close:
            if st.button("✕ Fermer", key="close_premium"):
                st.session_state["show_premium_modal"] = False; st.rerun()
    ...
    with tab1:
        if st.session_state["premium_livrable"]:
            ...
            <div class="livrable-auto">
                ...
                <div style="margin-top:6px;">Généré en {lv['duree']}s · Service : <b>{lv['service']}</b></div>
            </div>""", unsafe_allow_html=True)
            st.download_button(label="📥 TÉLÉCHARGER MON DOCUMENT", data=lv["buf"], file_name=lv["nom"], mime=lv["mime"], use_container_width=True)
            st.info("💡 Votre fichier est aussi disponible dans **📂 Mes Livrables** ci-dessus.")
            if st.button("🔄 Nouvelle mission", key="reset_livrable"):
                st.session_state["premium_livrable"] = None; st.rerun()
            st.stop()

        SERVICE_PREREQUIS = {
            "📝 Exposé scolaire complet IA": {
                "icone": "📝", "titre": "Exposé Scolaire Complet IA",
                ...
                "items": [("🎯","Le thème ou sujet de l'exposé"),("🏫","Le niveau scolaire (Collège, Lycée, Université...)"),("📏","Le nombre de pages désiré"),("🏢","L'école ou l'établissement concerné"),("📚","La matière ou discipline concernée")],
                "note": "Plus vos informations sont précises, plus le résultat sera adapté à vos attentes."
            },
            "📊 Data & Excel Analytics": {
                "icone": "📊", "titre": "Data & Excel Analytics",
                "intro": "Pour traiter vos données efficacement, merci de préciser dans votre cahier des charges :",
                "items": [("📁","Le type de fichier à traiter (Excel, CSV, autre...)"),("🎯","L'objectif de l'analyse (tableau de bord, graphiques, calculs...)"),("📋","Une description des données ou colonnes présentes"),("🔢","Le nombre approximatif de lignes ou d'entrées")],
                "note": "Vous pouvez également joindre un fichier exemple via WhatsApp après la soumission."
            },
            "⚙️ Pack Office (Word/Excel/PPT)": {
                "icone": "⚙️", "titre": "Pack Office — Word / Excel / PowerPoint",
                "intro": "Pour réaliser votre document Office au standard professionnel, précisez :",
                "items": [("📄","Le type de document souhaité (Word, Excel ou PowerPoint)"),("🎯","Le sujet ou contenu du document"),("📏","Le nombre de pages ou diapositives souhaité"),("🎨","Un style ou thème de couleurs si vous en avez un")],
                "note": "Précisez si vous souhaitez un logo ou une charte graphique particulière."
            },
            "🎨 Création Design IA": {
                "icone": "🎨", "titre": "Création Design IA",
                "intro": "Pour créer un design à la hauteur de vos attentes, indiquez-nous :",
                "items": [("🖼️","Le type de visuel (affiche, bannière, logo, flyer...)"),("📐","Le format ou dimensions souhaitées"),("🎨","Les couleurs ou thème visuel préféré"),("✍️","Les textes ou messages à intégrer"),("🏢","Le nom de votre entreprise ou projet")],
                "note": "Une référence visuelle ou exemple que vous aimez accélérera le travail."
            },
            "📚 Affiches & Reçus": {
                "icone": "📚", "titre": "Affiches & Reçus",
                "intro": "Pour concevoir votre affiche ou reçu, nous aurons besoin de :",
                "items": [("🏢","Le nom de votre entreprise ou organisation"),("📋","Les informations à faire apparaître (prix, date, lieu, contacts...)"),("🎨","La couleur principale ou identité visuelle"),("📐","Le format désiré (A4, A5, reçu thermique...)")],
                "note": "Un logo ou image à intégrer peut être envoyé via WhatsApp."
            },
            "👔 CV & Lettre de Motivation": {
                "icone": "👔", "titre": "CV & Lettre de Motivation",
                "intro": "Pour rédiger votre CV ou lettre de motivation de façon percutante, fournissez :",
                "items": [("👤","Votre nom complet et coordonnées"),("🎓","Vos diplômes et formations"),("💼","Vos expériences professionnelles"),("🎯","Le poste ou secteur visé"),("✨","Vos compétences clés et atouts")],
                "note": "Précisez si vous souhaitez uniquement le CV, la lettre, ou les deux."
            },
            "📄 Conversion & Fichier PDF": {
                "icone": "📄", "titre": "Conversion & Fichier PDF",
                "intro": "Pour traiter votre fichier correctement, indiquez-nous :",
                "items": [("📁","Le format du fichier source (Word, Excel, image, autre...)"),("🔄","Le format de sortie souhaité (PDF, Word, Excel...)"),("📋","Le nombre de fichiers à convertir"),("🔒","Si le PDF doit être protégé par mot de passe")],
                "note": "Envoyez votre fichier directement via WhatsApp après la soumission."
            },
            "📝 Création de Sujets & Examens": {
                "icone": "📝", "titre": "Création de Sujets & Examens",
                "intro": "Pour concevoir votre devoir ou examen sur mesure, veuillez nous préciser :",
                "items": [("🎓","Le niveau scolaire (Primaire, Collège, Lycée, Université...)"),("📚","La matière ou discipline concernée"),("🎯","Le type de sujet (devoir surveillé, examen, contrôle, concours...)"),("📏","Le nombre de questions ou d'exercices souhaité"),("⏱️","La durée prévue pour l'épreuve"),("🏢","L'établissement scolaire ou l'institution")],
        ...
        with col_f:
            ...
            service = st.selectbox("Type d'intervention", [
                "📊 Data & Excel Analytics", "📝 Exposé scolaire complet IA",
                "📝 Création de Sujets & Examens", "⚙️ Pack Office (Word/Excel/PPT)",
                "🎨 Création Design IA", "📚 Affiches & Reçus",
                "👔 CV & Lettre de Motivation", "📄 Conversion & Fichier PDF"
            ])
        with col_wa:
        ...
        if st.session_state["show_service_warning"] and service in SERVICE_PREREQUIS and service != SERVICE_SAISIE:
            ...
            st.info(f"""**{info["icone"]} {info["titre"]} — Informations requises**\n\n{info["intro"]}\n\n{"".join(f"- {icone} {texte}\n" for icone, texte in info["items"])}\n💡 *{info["note"]}*""")
            audio_file = SERVICE_AUDIO.get(service)
            if audio_file and os.path.exists(audio_file):
                with open(audio_file, "rb") as f:
                    b64 = __import__('base64').b64encode(f.read()).decode()
                components.html(f"""<script>(function(){{var binary=atob("{b64}");var bytes=new Uint8Array(binary.length);for(var i=0;i<binary.length;i++)bytes[i]=binary.charCodeAt(i);var blob=new Blob([bytes],{{type:"audio/mpeg"}});var audio=new Audio(URL.createObjectURL(blob));audio.volume=1;audio.play().catch(function(e){{}});}})();</script>""", height=0)
        ...
        if service == SERVICE_SAISIE and service in SERVICE_PREREQUIS:
            ...
            if st.session_state["show_service_warning"]:
                ...
                if os.path.exists("prerequis_excel.mp3"):
                    ...
                    components.html(f"""<script>(function(){{var binary=atob("{b64_excel}");var bytes=new Uint8Array(binary.length);for(var i=0;i<binary.length;i++)bytes[i]=binary.charCodeAt(i);var blob=new Blob([bytes],{{type:"audio/mpeg"}});var audio=new Audio(URL.createObjectURL(blob));audio.volume=1;audio.play().catch(function(e){{}});}})();</script>""", height=0)
                col_mid = st.columns([1, 2, 1])[1]
                with col_mid:
                    if st.button("✅ J'ai compris, je continue ma demande", key="close_service_warning_2"):
                        st.session_state["show_service_warning"] = False
                        components.html("<script>window.speechSynthesis.cancel();</script>", height=0)
                        st.rerun()
        ...
        champs_manquants = []
        if not wa_display: champs_manquants.append("WhatsApp de contact")
        if not prompt:     champs_manquants.append("Cahier des charges")
        if champs_manquants:
            st.markdown(f"""
        ...
        if premium_actif and service in SERVICES_GEMINI:
            st.markdown("""
            <div style="background:linear-gradient(135deg,rgba(255,215,0,.1),rgba(255,140,0,.06));border:1px solid rgba(255,215,0,.5);border-radius:12px;padding:12px 18px;margin:10px 0;">
        ...
        if st.button(label_btn):
            if not user:
                ...
                def generer():
                    try:
                        ...
                        if contenu.startswith("❌"):
                            result_holder["erreur"] = contenu; return
                        ...
                        result_holder["buf"] = buf; result_holder["nom"] = nom; result_holder["mime"] = mime
                    except Exception as e:
                ...
                if "erreur" in result_holder:
                    ...
                    new_req = {"id": hashlib.md5(str(datetime.now()).encode()).hexdigest()[:8], "user": user, "service": service,
                               "desc": prompt, "whatsapp": normalize_wa(wa_display), "status": "Traitement Nova en cours...",
                               "incomplet": False, "champs_manquants": [], "timestamp": str(datetime.now())}
                    ...
                    st.session_state["premium_livrable"] = {
                        "buf": result_holder["buf"], "nom": result_holder["nom"],
                        "mime": result_holder["mime"], "service": service, "duree": duree,
                    }
                    st.session_state["db"] = load_db()
                    st.rerun()

            else:
                st.session_state["is_glowing"] = True; st.rerun()

        if st.session_state["is_glowing"]:
            progress_placeholder = st.empty(); status_text = st.empty()
            bar = progress_placeholder.progress(0)
            for percent_complete in range(100):
                time.sleep(0.02); bar.progress(percent_complete + 1)
                status_text.markdown(f"<p style='text-align:center;color:{'#FFD700' if premium_actif else '#00d2ff'};font-size:1.2rem;font-weight:bold;'>NOVA PROCESSING : {percent_complete + 1}%</p>", unsafe_allow_html=True)
            ...
            new_req = {
                ...
                "user": user if user else "guest", "service": service,
                "desc": prompt if prompt else "(aucune description fournie)",
                "whatsapp": normalize_wa(wa_display) if wa_display else "(non renseigné)",
                "status": statut, "incomplet": bool(champs_manquants),
                "champs_manquants": champs_manquants, "timestamp": str(datetime.now())
            }
            st.session_state["db"]["demandes"].append(new_req)
            save_demande(new_req)
            envoyer_notification(user if user else "Visiteur", normalize_wa(wa_display) if wa_display else "(non renseigné)", service, prompt if prompt else "(aucune description fournie)")
            st.session_state["db"] = load_db()
            st.session_state["is_glowing"] = False
            progress_placeholder.empty(); status_text.empty()
            if user:
                ...
                if os.path.exists(audio_path_confirm):
                    ...
                    components.html(f"""<script>(function(){{var b64="{audio_b64_confirm}";var binary=atob(b64);var bytes=new Uint8Array(binary.length);for(var i=0;i<binary.length;i++)bytes[i]=binary.charCodeAt(i);var blob=new Blob([bytes],{{type:"audio/mpeg"}});var audio=new Audio(URL.createObjectURL(blob));audio.volume=1;audio.play().catch(function(e){{}});}})();</script>""", height=0)
                st.balloons(); time.sleep(20); st.rerun()
            else:
                st.session_state["view"] = "auth"; st.rerun()
        ...
        if not user:
            ...
            st.markdown("""
                <div style="background:rgba(46,204,113,0.1);padding:15px;border-radius:10px;border:1px dashed #2ecc71;margin-bottom:20px;text-align:center;">
                    ...
                    <p style="font-size:0.9rem;">Accédez à vos actifs numériques terminés.</p>
                </div>
            """, unsafe_allow_html=True)
            ...
            if st.session_state["premium_livrable"]:
                ...
                <div class="livrable-auto">
                    ...
                    <div style="margin-top:4px;">Généré en {lv['duree']}s · {lv['service']}</div>
                </div>""", unsafe_allow_html=True)
                st.download_button("📥 TÉLÉCHARGER MON DOCUMENT", data=lv["buf"], file_name=lv["nom"], mime=lv["mime"], use_container_width=True)
                st.divider()
                ...
                for link in user_links:
                    if link["url"].startswith("__local__"):
                        ...
                        <div class="file-card">
                            <div style="display:flex;justify-content:space-between;align-items:center;">
                                <div style="padding-left:8px;">
                                    <h3 style="color:{'#FFD700' if premium_actif else '#00d2ff'};margin:0;">💎 {link['name']}</h3>
                                    <p style="color:#aaa;font-size:0.85rem;margin:5px 0;">Finalisé le {link.get('date',"Aujourd'hui")}</p>
                                </div>
                                <a href="{link['url']}" target="_blank" style="text-decoration:none;">
                                    <button style="padding:10px 25px;background:{'linear-gradient(90deg,#b8860b,#FFD700)' if premium_actif else '#2ecc71'};color:{'#000' if premium_actif else 'white'};border:none;border-radius:30px;font-weight:bold;cursor:pointer;">
                                        📥 TÉLÉCHARGER
                                    </button>
                                </a>
                            </div>
            ...
            if user_reqs:
                ...
                for r in user_reqs:
                    st.markdown(f"""
                        <div class="file-card" style="border-left:5px solid {'#FFD700' if premium_actif else '#f1c40f'};border-color:rgba(241,196,15,0.3);">
                            <div style="display:flex;justify-content:space-between;align-items:center;">
                                <div style="padding-left:8px;">
                                    <strong style="color:{'#FFD700' if premium_actif else '#f1c40f'};">{r['service']}</strong><br>
                                    <span style="color:#eee;font-size:0.9rem;">Status: {r['status']}</span>
                                </div>
                                <div style="width:20px;height:20px;border:3px solid rgba(255,255,255,0.1);border-top:3px solid {'#FFD700' if premium_actif else '#f1c40f'};border-radius:50%;animation:spin 1s linear infinite;"></div>
                            </div>
                        </div>
            ...
            with admin_tab1:
                ...
                for i, req in enumerate(current_db["demandes"]):
                    ...
                    if est_incomplet and champs_manquants:
                        ...
                        msg_rejet = (f"Bonjour {client_nom}, nous avons reçu votre demande Nova AI concernant : {service}. "
                                     f"Cependant, nous ne pouvons pas la traiter car les informations suivantes sont manquantes : {champs_str}. "
                                     f"Merci de soumettre à nouveau votre demande en complétant tous les champs. — Équipe Nova AI ⚡")
                    else:
                        msg_rejet = (f"Bonjour {client_nom}, nous avons bien reçu votre demande Nova AI concernant : {service}. "
                                     f"Malheureusement, nous ne sommes pas en mesure de traiter cette mission pour le moment. "
                                     f"Merci de nous recontacter. — Équipe Nova AI ⚡")
                    ...
                    msg_recu   = (f"📬 Bonjour {client_nom}, nous confirmons la réception de votre demande Nova AI : {service}. "
                                  f"Votre mission est en cours de traitement. "
                    ...
                    if service in SERVICES_GEMINI:
                        ...
                        if st.button(f"🔍 Voir modèles disponibles", key=f"diag_{req_id}"):
                            ...
                            if modeles_dispo:
                                ...
                                for m in modeles_dispo: st.code(m)
                            else:
                        ...
                        if st.button(f"⚡ APPROUVER & GÉNÉRER AVEC GEMINI", key=f"gemini_{req_id}", use_container_width=True):
                            with st.spinner("🔍 Détection automatique du meilleur modèle disponible..."):
                                modeles_dispo = get_modeles_disponibles(st.secrets["GEMINI_API_KEY"])
                                if modeles_dispo: st.info(f"✅ Modèle sélectionné : **{modeles_dispo[0]}**")
                                else: st.error("❌ Aucun modèle Gemini disponible pour cette clé API.")
                            with st.spinner("🤖 Gemini génère le document... (30-60 secondes)"):
                                contenu = generer_avec_gemini(service, description, client_nom)
                            if contenu.startswith("❌"):
                                st.error(contenu)
                            else:
                                st.session_state["gemini_results"][req_id] = {"contenu": contenu, "service": service, "client": client_nom}
                                st.success("✅ Document généré avec succès !"); st.rerun()
                        ...
                        if req_id in st.session_state["gemini_results"]:
                            ...
                            try:
                                if result["service"] == "📊 Data & Excel Analytics":
                                    buf = creer_xlsx(result.get("desc", ""), result["client"])
                                    nom_fichier = f"NovaAI_{client_nom}_Suivi_Depenses.xlsx".replace(" ", "_")
                                    st.download_button("📥 TÉLÉCHARGER LE FICHIER EXCEL (.xlsx)", data=buf, file_name=nom_fichier, mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", key=f"dl_{req_id}", use_container_width=True)
                                    ...
                                    st.download_button("📥 TÉLÉCHARGER LE DOCUMENT WORD (.docx)", data=buf, file_name=nom_fichier, mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document", key=f"dl_{req_id}", use_container_width=True)
                            except Exception as e:
                    ...
                    if st.button("📦 LIVRER LA MISSION AU CLIENT", key=f"btn_{i}", use_container_width=True):
                        if url_dl:
                            ...
                            st.session_state["db"] = load_db()
                            st.success(f"✅ Mission livrée à {client_nom} !"); st.rerun()

            # ── ONGLET GESTION PREMIUM — PROTÉGÉ PAR MOT DE PASSE 1309 ──
            with admin_tab2:

                if not st.session_state["premium_admin_unlocked"]:
                    # ── ÉCRAN DE VERROUILLAGE DORÉ ───────────────────────
                    st.markdown("""
                    <div style="
                        background: linear-gradient(145deg, rgba(20,15,0,0.97), rgba(30,22,0,0.95));
                        border: 2px solid #FFD700;
                        border-radius: 22px;
                        padding: 40px 30px;
                        text-align: center;
                        margin: 20px 0;
                        box-shadow: 0 0 60px rgba(255,215,0,0.15), inset 0 0 40px rgba(255,215,0,0.03);
                        position: relative;
                        overflow: hidden;
                    ">
                        <div style="position:absolute;top:0;left:0;right:0;height:3px;
                            background:linear-gradient(90deg,transparent,#b8860b,#FFD700,#FFF5C0,#FFD700,#b8860b,transparent);"></div>
                        <div style="font-size:3rem;margin-bottom:15px;">🔐</div>
                        <div style="font-size:1.5rem;font-weight:800;color:#FFD700;letter-spacing:2px;margin-bottom:8px;
                            text-shadow:0 0 15px rgba(255,215,0,0.5);">
                            ZONE NOVA GOLD
                        </div>
                        <div style="color:rgba(255,215,0,0.6);font-size:0.9rem;margin-bottom:25px;">
                            Cette section est protégée. Entrez le code d'accès Gold pour continuer.
                        </div>
                    </div>
                    """, unsafe_allow_html=True)

                    col_lock = st.columns([1, 2, 1])[1]
                    with col_lock:
                        code_saisi = st.text_input(
                            "🔑 Code d'accès Gold",
                            type="password",
                            placeholder="••••",
                            key="premium_admin_input"
                        )
                        if st.button("🔓 DÉVERROUILLER", key="btn_unlock_premium"):
                            if code_saisi == PREMIUM_ADMIN_CODE:
                                st.session_state["premium_admin_unlocked"] = True
                                st.success("✅ Accès Gold autorisé !")
                                st.rerun()
                            else:
                    ...
                    <div style="text-align:center;margin-top:15px;">
                        <span style="color:rgba(255,215,0,0.25);font-size:0.75rem;letter-spacing:2px;">
                            ✦ NOVA GOLD ADMIN ✦ ACCÈS RESTREINT ✦
                        </span>
                    </div>""", unsafe_allow_html=True)

                else:
                    # ── INTERFACE GESTION PREMIUM DÉVERROUILLÉE ──────────
                    col_title, col_lock_btn = st.columns([4, 1])
                    with col_title:
                        st.markdown("### 👑 Gestion des membres Premium")
                    with col_lock_btn:
                        st.markdown("<br>", unsafe_allow_html=True)
                        if st.button("🔒 Verrouiller", key="btn_relock"):
                            st.session_state["premium_admin_unlocked"] = False
                            st.rerun()
                    ...
                    with co3:
                        ...
                        if st.button("⚡ ACTIVER", key="btn_act_global"):
                            ...
                            st.session_state["db"] = load_db()
                            st.success(f"✅ Premium **{plan_ch}** activé pour **{uid_target}** !"); st.rerun()
                            ...
                            if p_actif:
                                ...
                                with cp1:
                                    ...
                                    if st.button("➕ Prolonger", key=f"pro_{uid_m}"):
                                        ...
                                        st.session_state["db"] = load_db()
                                        st.success(f"✅ Prolongé jusqu'au {new_exp.strftime('%d/%m/%Y')} !"); st.rerun()
                                with cp2:
                                    st.markdown("<br>", unsafe_allow_html=True)
                                    if st.button("🗑️ Révoquer", key=f"rev_{uid_m}"):
                                        desactiver_premium(uid_m)
                                        st.session_state["db"] = load_db()
                                        st.warning(f"Premium révoqué pour {uid_m}."); st.rerun()
                            else:
                                ...
                                if st.button("⚡ Activer", key=f"actbtn_{uid_m}"):
                                    ...
                                    st.session_state["db"] = load_db()
                                    st.success(f"✅ Premium activé pour {uid_m} !"); st.rerun()
...
inject_custom_css(is_premium=False)
