...
# ==========================================
# LOGIQUE DE DONNÉES (STABILITÉ TOTALE)
# ==========================================
...
# ==========================================
# DESIGN ET STYLE (CSS AVANCÉ - SANS CHANGEMENT)
# ==========================================
...
# ==========================================
# PAGES ET COMPOSANTS (NAVIGATION INTUITIVE)
# ==========================================
...
def main_dashboard():
    ...
    # INTERFACE DE LIVRAISON (CONSOLE ADMIN)
    with st.expander("🛠 Console Admin Nova"):
        if st.text_input("Master Key", type="password") == ADMIN_CODE:
            ...
            for i, req in enumerate(list(current_db["demandes"])):
                st.write(f"📦 **{req['user']}** - {req['service']}")
                url_dl = st.text_input(f"Lien {req['id']}", key=f"url_{i}")
                if st.button(f"LIVRER MISSION", key=f"btn_{i}"):
                    if url_dl:
                        user_target = req['user']
                        if user_target not in current_db["liens"]: current_db["liens"][user_target] = []
                        current_db["liens"][user_target].append({
                            "name": req['service'], 
                            "url": url_dl,
